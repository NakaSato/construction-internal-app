...
const ProjectDebug: React.FC = () => {
  ...
  useEffect(() => {
    const testApiConnection = async () => {
      setTestLoading(true);
      try {
        console.log("[TEST] [ProjectDebug] Testing API connection...");
  ...
  const testProjectById = async (projectId: string) => {
    try {
      console.log(`[TEST] [ProjectDebug] Testing project ID: ${projectId}`);
      const project = await projectsApi.getProjectById(projectId);
      console.log(`[SUCCESS] [ProjectDebug] Project found:`, project);
      navigate(`/projects/${projectId}`);
    } catch (err) {
      console.error(
        `[ERROR] [ProjectDebug] Failed to fetch project ${projectId}:`,
        err
      );
      alert(
        `Failed to fetch project ${projectId}: ${err instanceof Error ? err.message : "Unknown error"
        }`
      );
    }
  };
          ...
          <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span
                  className={`px-3 py-1 rounded-full text-sm font-medium ${isAuthenticated
              ...
              <div className="flex items-center justify-between">
                ...
                <span
                  className={`px-3 py-1 rounded-full text-sm font-medium ${token
                    ? "bg-blue-100 text-blue-800"
                    : "bg-red-100 text-red-800"
                    }`}
                >
          ...
          <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
            ...
            {testLoading ? (
              ...
              <div className="space-y-4">
                <div
                  className={`p-4 rounded-lg ${apiTest.success
                ...
                >
                  <div className="flex items-center mb-2">
                    <span
                      className={`text-2xl mr-2 ${apiTest.success ? "text-blue-600" : "text-red-600"
                        }`}
                    >
                      {apiTest.success ? (
                    ...
                    <span
                      className={`font-medium ${apiTest.success ? "text-blue-800" : "text-red-800"
                        }`}
                    >
                      {apiTest.success
            ...
            {loading ? (
              ...
              <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                <span className="text-red-600 flex items-center gap-2">
                  <XCircle className="h-4 w-4" /> Hook Error: {error}
                </span>
              </div>
            ) : (
              <div className="space-y-4">
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  <span className="text-blue-800 flex items-center gap-2">
                    <CheckCircle2 className="h-4 w-4" /> Hook loaded {projects.length} projects
                  </span>
                </div>
                ...
                {projects.length > 0 ? (
                  ...
                  <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                    <span className="text-yellow-800 flex items-center gap-2">
                      <AlertTriangle className="h-4 w-4" /> No projects found in hook data
                    </span>
                  </div>
                )}
              </div>
